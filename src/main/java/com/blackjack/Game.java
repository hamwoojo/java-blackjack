...
public class Game {
    ...
    public List<Player> initPhase(List<Player> players, CardDeck cardDeck){
    ...
    private boolean isAllPlayerTurnOff(List<Player> players) {
        return players.stream().anyMatch(player -> player.isTurn());
    }
    ...
    private List<Player> receiveCardAllPlayers(Scanner sc, CardDeck cardDeck, List<Player> players) {
        players.stream()
            .peek(player -> System.out.println(player.getName()+"님 차례입니다."))
            .forEach(player -> {
                if(isReceiveCard(sc)){
                    Card card = cardDeck.draw();
                    player.receiveCard(card);
